...
typedef struct tagIHW265D_INIT_PARAM
{
    ...
    INT32   iMaxWidth;                   // [in] maximum width,[8,8192]
    INT32   iMaxHeight;                  // [in] maximum height,[8,8192]
    INT32   iMaxRefNum;                  // [in] maximum reference num,[0,15]
	INT32   iMaxVPSNum;                  // [in] maximum vps num,[1,16]
	INT32   iMaxSPSNum;                  // [in] maximum sps num,[1,16]
	INT32   iMaxPPSNum;                  // [in] maximum pps num,[1,64]

	UINT32  uiBitDepth;                // [in]bitdepth,8 or 10
    BOOL32  uiMvcEnable;               // 1: MVC Enable; 0£ºMVC Disable

    HW265D_THREADTYPE  eThreadType;  // thread type,0: singal thread; 1: multi thread
...
typedef struct tagIH265DEC_INARGS
{
    ...
    UINT32 uiStreamLen;
    UINT64 uiTimeStamp;
    HW265D_DECODEMODE eDecodeMode; // 0:DEC_FRAME;1:DEC_AU
...
typedef struct tagIH265DEC_OUTARGS
{
	...
	UINT32              uiBitDepthY;               //luma bitdepth
	UINT32              uiBitDepthC;               //chroma bitdepth
	INT32                 iDisplayPrimariesX0;     //hdr10 luma x coordinate
	INT32                 iDisplayPrimariesX1;     //hdr10  Cb  x coordinate
	INT32                 iDisplayPrimariesX2;     //hdr10  Cr  x coordinate
	INT32                 iDisplayPrimariesY0;     //hdr10 luma y coordinate
    INT32                 iDisplayPrimariesY1;     //hdr10  Cb  y coordinate
	INT32                 iDisplayPrimariesY2;     //hdr10  Cr  y coordinate
	INT32                 iWhitePointX ;           //hdr10 white point x coordinate
	INT32                 iWhitePointY ;           //hdr10 white point y coordinate
	INT32                 iMaxDisplayMasteringLuma; //max hdr10 luma level
	INT32                 iMinDisplayMasteringLuma; //min hdr10 luma level
	INT32                 iMaxContentLightLevel;    //max hdr10 content light level
	INT32                 iMaxPicAverageLightLevel; //min hdr10 content light level
...
typedef enum tagIHW265D_RETURN
{
	...
	IHW265D_INVALID_MAX_VPS_NUM,                        // maximum vps num exceed limit
	IHW265D_INVALID_MAX_SPS_NUM,                        // maximum sps num exceed limit
	IHW265D_INVALID_MAX_PPS_NUM,                        // maximum pps num exceed limit
	...
	IHW265D_BITDEPTH_ERROR    			= 0xE0404009     // bitdepth error
